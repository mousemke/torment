...
const debug = require('debug')('torment');
...
class TormentTestRunner
{
  ...
  * test(url, fn) {
  ...
  /**
  * exposes the nightmare wait function
  *
...
class Torment {
  ...
  constructor(base, test, options, fn) {
    if (!options.targets)
    {
      console.warn('Testing from the torment object is DEPRECATED and will be removed with version 1.0.0. Please define targets');

      return new TormentTestRunner(base, test, options);
    }
    ...
    this.testers = Object.keys(targets).map(label => {
      ...
      const torment = new TormentTestRunner(
        base,
        test,
        Object.assign({
          width,
